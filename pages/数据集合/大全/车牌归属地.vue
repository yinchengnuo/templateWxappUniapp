      ...
      <template v-if="page">
        <view class="flex margin-top-xs">
          <scroll-view class="cu-vertical-nav nav cu-timeline" scroll-y scroll-with-animation :scroll-top="VerticalNavTop1" style="width: 200rpx" :style="{ height: page.height + 'px' }">
            <view class="cu-item" style="padding: 0" :class="index == TabCur1 ? 'text-black text-bold cur' : 'text-black'" v-for="(item, index) in list" :key="index" @click="tabSelect1" :data-id="index">{{ item.title }} </view>
          </scroll-view>
            ...
            <view class="padding-top-xs padding-lr-xs" v-for="(item, index) in list" :key="index" :id="'main-1-' + index">
              <view class="cu-bar solid-bottom bg-white" style="position: sticky; top: 0; z-index: 1; min-height: 80rpx">
                <view class="action">
                  ...
                  <text class="text-bold text-black">{{ item.title }}</text>
                </view>
                <view class="action">{{ item.list.length }}</view>
              </view>
              <view class="padding-xs">
                <view v-for="(itemm, indexx) in item.list" :key="indexx" class="sm-border flex flex_sb bg-white shadow radius margin-xs padding-sm">
                  <view class="text-lg text-bold">{{ itemm.label }}</view>
                  <view class="text-lg">{{ itemm.title }}</view>
                </view>
              </view>
            </view>
          </scroll-view>
