			...
			<template v-if="page">
				...
				<view class="cu-bar input solid-bottom">
					<input v-model.trim="text" ref="input" :focus="focus" class="my_input" confirm-type="search"
						placeholder="请输入关键字" type="text" @blur="focus = false" @confirm="make" />
				...
				<template v-if="result">
					<view class="cu-list menu sm-border card-menu">
						<view class="flex justify-between flex-wrap">
							<image class="margin-top-xs margin-bottom-xs" v-for="(item, index) in result" :key="item[0]" :src="item[0]"
								style="width: 222rpx; height: 222rpx; border-radius: 28rpx; overflow: hidden;" show-menu-by-longpress
								mode="aspectFill" @click="$preview(result)">
							</image>
						</view>
...
export default {
	...
	methods: {
		...
		make() {
			...
			this.$loading()
			uni.request({
				url: 'https://api.oioweb.cn/api/picture/emoticon?keyword=' +
					(this.text || 'Hi')
			}).then((res) => {
				console.log(res)
