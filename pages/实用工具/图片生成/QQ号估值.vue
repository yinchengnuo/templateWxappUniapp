    ...
    <template v-slot:default="{ page }">
      <template v-if="page">
        ...
        <view class="cu-bar input solid-bottom">
          <input v-model.trim="text" ref="input" :focus="focus" class="my_input" confirm-type="search" placeholder="请输入单号" type="digit" @blur="focus = false" @confirm="make" />
          <text
            v-if="text"
        ...
        <template v-if="result">
          <view class="cu-list menu sm-border card-menu margin-top margin-bottom" @click="$preview(result)">
            ...
            <image v-else :src="result" mode="aspectFill" show-menu-by-longpress style="width: 686rpx; height: 529rpx" @load="load" @error="error"> </image>
          </view>
        </template>
