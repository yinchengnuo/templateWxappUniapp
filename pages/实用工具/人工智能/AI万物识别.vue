				...
				<template v-if="result">
					...
					<view class="cu-list menu sm-border card-menu margin-top margin-bottom">
						<block v-for="(item, index) in result" :key="index">
							...
							<view v-if="item.baike_info && item.baike_info.image_url" class="cu-item"
								@click="$preview(item.baike_info.image_url)">
	...
	methods: {
		chooseImg() {
			this.$choose().then(({ tempFiles: [{ tempFilePath: filePath }] }) => {
				this.$loading()
				this.text = filePath
				uni.uploadFile({ filePath, name: 'file', url: 'https://api.oioweb.cn/api/ocr/recognition' }).then(({ data }) => {
