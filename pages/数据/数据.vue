	...
	<view class="index">
		<view class='nav-list'>
			<view v-for="(item, index) in list" :key="index" :class="['nav-li', 'shadow']"
				...
				<view class="text-white padding-xs text-bold text-shadow padding-xs w100"
					style="z-index: 0; white-space: nowrap; line-height: 2; position: absolute; bottom: 0; left: 0; background: linear-gradient(rgba(255, 255, 255, .05), rgba(255, 255, 255, .4));">
					<view class="flex flex_sb">
						<view class="cuIcon-peoplefill text-progress"><text style="font-weight: bold;">{{
							item.user_num
						}}</text></view>
						<view class="cuIcon-favorfill text-orange"><text style="font-weight: bold;">{{
							item.collect_num
						}}</text></view>
						<text class="cuIcon-attentionfill text-black"><text style="font-weight: bold">{{
							item.view_num }}</text>
						</text>
					</view>
				</view>
