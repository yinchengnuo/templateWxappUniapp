...
export default {
	data() {
		return {
			...
			content: [{ selectedIconPath: '/static/icons/升序.png', text: '默认排序', active: true },
	...
	computed: {
		...
		listHot() {
			...
			return this.list.slice().sort((b, a) => a.view_num - b.view_num).slice(0, 6).map((e, i) => ({ ...e, image: images[i] }))
		},
		listNew() {
			return this.list.slice().sort((b, a) => a.index - b.index).slice(0, 6)
		},
		listDaily() {
			return this.$store.state.app.list.filter(e => e.type === '每日随机').slice().sort((b, a) => a.view_num - b.view_num)
		}
	},
	onShow() {
		this.show++
