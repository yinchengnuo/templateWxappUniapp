	...
	<view>
		...
		<view v-if="result" class="cu-list menu">
			<block v-for="(item, index) in result.content" :key="index">
				...
				<view v-if="item.dict" class="cu-item text-black" :class="item.checked ? '' : 'piano-hidden-content'"
					v-for="(itemm, indexx) in item.list" :key="indexx" @click="clickDetail(result.name, item.dict, itemm.name)">
...
export default {
	...
	created() {
		this.$loading()
		this.$('/book_dict', { name: getCurrentPages().at(-1).route.split('/').at(-1) }).then(data => {
