<template>
	<view class="color-picker" :class="{ show: isShow === true, hide: isShow === false, under: !isShow && !showKey }"
		@tap="hide()">
		<view class="picker-content" :class="{ show: isShow === true, hide: isShow === false }" @tap.stop="">
			<view class="action">
				<text class="text" @tap="hide">取消</text>
				<text class="tex theme-color" @tap="colorPickerChoose">确定</text>
			</view>
			<view class="picker">
				...
				<view class="color-view" @touchstart="pickColor" @touchmove="pickColor"
					:style="{ backgroundColor: 'hsl(' + hueView.H + ', 100%, 50%)' }">
					<text class="anchor"
						:style="{ top: colorView.anchorTop + 'px', left: colorView.anchorLeft + 'px' }"></text>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			pickColor(e) { // 选择颜色
				...
				this.colorView.L = this.floor((1 - this.colorView.S * 0.5) - this.colorView.anchorTop / this.colorView
					.height / (this.colorView.S + 1))
				this.colorName = this.getColorString() // 根据 HLS 计算 RGB 字符串
